...
def test():
    # Tekst, dla którego chcesz wygenerować osadzenie
    texts = ["Przykładowy tekst 1", "Inny przykład tekstu"]

    # Wywołanie API do generowania osadzeń
    response = openai.embeddings.create(
        model="text-embedding-ada-002",  # Model rekomendowany do osadzeń
        input=texts
    )

    # Pobranie osadzeń
    embeddings = [data["embedding"] for data in response.to_dict()["data"]]
